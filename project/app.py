...
def handle_rc5_encrypt(W, R):
    st.subheader(f"{TAB_ENCRYPT} using RC5-CBC-Pad")
...
def handle_rc5_decrypt(W, R):
    st.subheader(f"{TAB_DECRYPT} using RC5-CBC-Pad")
...
def show_rc5():
    ...
    tab_enc, tab_dec = st.tabs([TAB_ENCRYPT, TAB_DECRYPT])
    
    with tab_enc:
        handle_rc5_encrypt(W, R)
    with tab_dec:
        handle_rc5_decrypt(W, R)
...
def handle_rsa_encrypt():
    st.subheader(f"{TAB_ENCRYPT} using RSA")
    ...
    if st.button("Encrypt File", type="primary", use_container_width=True, key="btn_rsa_enc"):
...
def handle_rsa_decrypt():
    st.subheader(f"{TAB_DECRYPT} using RSA")
    ...
    if st.button("Decrypt File", type="primary", use_container_width=True, key="btn_rsa_dec"):
...
def show_rsa():
    st.header(LAB4_NAME)
    tab_keys, tab_enc, tab_dec, tab_speed = st.tabs(["Manage Keys", TAB_ENCRYPT, TAB_DECRYPT, "Compare Speed (RC5 vs RSA)"])
...
def handle_dsa_sign():
    ...
    if st.button("Sign Data", type="primary", use_container_width=True, key="btn_dsa_sign"):
...
def handle_dsa_verify():
    ...
    if st.button("Verify Signature", type="primary", use_container_width=True, key="btn_dsa_verify"):
...
def main():
    with st.sidebar:
        ...
        page = st.radio(
            "Navigation:",
            [LAB1_NAME, LAB2_NAME, LAB3_NAME, LAB4_NAME, LAB5_NAME]  
        )
    if page == LAB1_NAME:
        show_lcg()
    if page == LAB2_NAME:
